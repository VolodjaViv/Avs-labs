...
void print_time(clock_t start, clock_t end, string type) {
    if (type == "ompOff") { cout << "Straight_result_time: " << (double)(end - start) / CLK_TCK << endl; }
    else if (type == "ompOn") { cout << "Dynamic_parallel_result_time: " << (double)(end - start) / CLK_TCK << endl; }
}
